...
<html>
	...
	<body>
		...
		<div class="f">
			<form class="form" method="POST" action="/jumex/{{$jumex->id}}" id="formulario">
			@csrf
			@method('patch')
				<div class="mb-3">
					<label for="Nombre" class="form-label">Nombre</label>
					<input type="text" class="form-control" name="Nombre" id="Nombre" value="{{$jumex->Nombre}}">
				</div>
				<div class="mb-3">
					...
					<select class="form-select" aria-label="Default select example" name="Sabores[]" id="Sabores" multiple>
						...
						<option value="{{$Sabores->id}}" @if ($jumex->Sabores->contains($Sabores->id)) selected @endif>{{ $Sabores->Sabores }}</option>
						@endforeach
                	</select>
            	</div>
				...
				<div class="mb-3">
					...
					<input type="text" class="form-control" name="Categoria" id="Categoria" value="{{$jumex->Categoria}}">
				</div>
				<div class="mb-3">
					<label for="Precio" class="form-label">Precio</label>
					<input type="text" class="form-control" name="Precio" id="Precio" value="{{$jumex->Precio}}">
				</div>
